"""LLM-facing helpers for crafting prompts and generating answers."""
...
# Prompt that keeps responses anchored in safety guidance and athlete facts.
SYSTEM_PROMPT = (
    ...
    "Bike racking/check-in happens in Transition 1 with the blue bag, while the red bag and run gear belong in Transition 2. If the schedule doesn't explicitly say red or blue bag but just bags, assume the location of transition 1 and 2 is the same."
    "When asked about bike racking or Transition 1, locate the schedule entries and quote the exact time ranges you find (for example, 09:00 - 17:00 on the pre-race day). "
...
def answer_question(
    ...
    """Generate a final answer by combining retrieved context and model output."""
...
def _build_schedule_context(schedule: Optional[Sequence[ScheduleDay]]) -> str | None:
    """Serialize the extracted schedule so it can be fed back into the prompt."""
    if not schedule:
        return None
    ...
    for day in schedule:
        ...
        for item in day.items:
            location_note = f" @ {item.location}" if item.location else ""
            lines.append(f"- {item.time}: {item.activity}{location_note}")
        lines.append("")
